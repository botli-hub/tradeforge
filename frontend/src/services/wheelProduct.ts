...
export function getPortfolioBudgetSource(): BudgetSource {
  try {
    const s = localStorage.getItem(BUDGET_SOURCE_KEY)
    if (s === 'nav' || s === 'config' || s === 'legacy' || s === 'default') return s
  } catch { /* */ }
  try {
...
/** 仅内部/测试写入缓存;业务修改请走设置页起始现金,活权益由后端覆盖 */
export function setPortfolioBudget(v: number, source: BudgetSource = 'legacy') {
...
/** 用后端活权益覆盖预算。 */
export function syncPortfolioBudgetFromNav(equity: number | null | undefined): number | null {
  const eq = equity != null && Number.isFinite(Number(equity)) ? Number(equity) : 0
  if (eq > 0) {
    setPortfolioBudget(eq, 'nav')
    return eq
  }
  return null
}
...
export const STRATEGY_TEMPLATES: Record<RiskTier, {
  ...
  balanced: {
    label: '均衡',
    desc: '愿接约 0.90×现价;默认 Wheel 舒适区 10–55 DTE。Put strike≤floor;Call 看成本底线',
    floor_pct_of_spot: 0.90,
    delta_min: 0.15, delta_max: 0.30,
    dte_min: 10, dte_max: 55,
    min_annualized: 15,
    profit_target_pct: 50,
