...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "wheel_timing": {
        ...
        # 信号桶冷却键= SYMBOL|PUT|1h|EMA50(同桶共享;扫描+TG 均跳过)。
        # 语义=同一美股交易日不重复(存 session 日);下一 RTH open 解冻。
        # cooldown_trading_days 已弃用(信号桶路径忽略,保留兼容旧配置/调用)。
    ...
    # IV 环境(自动=启用标的中位 IVR);叠加开仓/止盈频率,侧重组合年化周转
    "wheel_iv_regime": {
