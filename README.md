...
- **优先开仓**：高分扫描 ∩ 触线时机；只有新鲜 bid/ask、点差合格且组合风控通过才标为可执行
- **持股待挂 CC**：仅 HOLDING。发现走合约 1h 触线（无触线不算时机到）；立场只升档。DTE 默认 10–55。
- **愿卖价** `sell_above` 是 CC strike 锚，与 Put 愿接价 `floor_price` 分开
- **资金**：权益 = 起始现金 ± 登记现金流 + 持股市值 − 空头期权盯市。起始现金只取组合设置，不用各标的上限猜余额；缺行情会标记估值不完整并暂停计划新交易。
...
默认阈值（可在设置 `wheel_position` 覆盖）包括：硬止盈 50%、软止盈 30%、过高持有 80%、吃 θ 40%/14DTE/年化 12%、硬处理窗 21DTE、临期 7DTE、深 ITM Put Δ0.38 / Call Δ0.35 或价内 3%、薄垫 1.5%、资金紧利用率 75%、Put **strike ≤ 愿接价 floor**。剩余年化按占用权益（Put=担保 strike，Call=持股成本/市值）。几天内打到止盈且剩余 DTE 仍长则落袋周转。持仓看 IV 分位（塌缩优先止盈，高位更愿吃 θ）。Call 权利金不值覆盖则揭盖，不把换仓和揭盖混为一谈。

轮子列表 **平仓 / 管理** → 先开**决策弹窗**（接货/买回/Roll 三列数字），再决定登记。点差过宽不能按 ask 宣称止盈。

### 机会流
- **触线时机**：Put 合约日K / Call 合约 1h 穿 EMA50/200；Call 发现以此为准
- **全池高分扫描**：年化 × 流动性 × 趋势 × 财报 × IV × POP 等
- 统一机会流：双轨对齐、档位（优先/可排/观察）、组合闸门

### 台账与执行
- 状态机：`IDLE → CSP → HOLDING → CC → CLOSED`，按实际数量保留部分平仓/指派、多个 CC 腿和费用/P&L
- 一键执行草稿（买回 / 到期 / 接货 / Roll 两腿）；批量登记原子提交，`execution_id` 重试幂等
- Roll 对比与登记
...
|------|------|
| 行情 | 搜索、quote、K 线、观察池 |
| 缠论 | 选定标的与级别后分解：笔 / 线段 / 中枢 / 走势类型 / 三类买卖点 |
| 期权 | Futu 链、Greeks、收益图 |
| 策略 / Formula | 可视化 + DSL 校验/转译、信号评估 |
| 回测 | 策略回测；Wheel HV 情景模拟（含空头期权负债、费用、滑点），历史 bid/ask 才能做时机对照 |
...
2. 设置 → **Wheel**：起始现金、标的愿接价在 Wheel→标的页维护  
...
| POST | `/api/wheel/backtest/timing-compare` | 同一历史报价样本对照 EMA 时机；无历史报价不宣称优势 |
